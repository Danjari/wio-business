...
2. Extracts fields via AWS Textract (primary) → Claude Haiku text (fallback) → Claude Sonnet vision (last resort)
3. Categorizes via two independent methods: rule-based (keyword dict + TF-IDF) and LLM (Claude Haiku)
...
backend/
  ...
  pipeline/
    ...
    llm_text.py              — Claude Haiku text fallback
    llm_vision.py            — Claude Sonnet vision last resort
...
# Categorization agreement (needs ANTHROPIC_API_KEY for LLM method)
...
|---|---|---|
| Textract field confidence | < 80 (Textract 0-100 scale) | Trigger Claude Haiku text fallback |
| Claude Haiku text confidence | < 0.99 | Trigger Claude Sonnet vision |
| Claude Sonnet vision confidence | < 0.99 | Ask user for clearer photo |
